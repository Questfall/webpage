...
export tag Screen
	...
	def mount
		...
		scroll.onfinish = do $container.flags.add('basic').remove('perspective')
	
	css
		.perspective
			transform: perspective(2000px) rotateY(15deg) rotateZ(-15deg) rotateX(55deg)
		.basic
			animation: basic 1s forwards
			@keyframes basic
				0% transform: perspective(2000px) rotateY(15deg) rotateZ(-15deg) rotateX(55deg)
				100% transform: perspective(2000px) rotateY(0deg) rotateZ(0deg) rotateX(0deg)
	<self>
		css aspect-ratio:{1400 / 1065} transform-style:preserve-3d scale:0.8 will-change:transform
		<div$container .vcontent .perspective>
